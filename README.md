...
| #   | Description | Comment | Attacks/Reports | Category |
| --- | ----------- | ------- | --------------- | -------- |
| 8.1 | Verify that private keys are never left unencrypted at rest on any storage device. If an encrypted private key is stored as a file, it should be protected with appropriate file-system permissions (e.g., 0600 or 0400). If an encrypted private key is stored in a separate key-management service, it should have access controls that restrict unauthorized access. | Secure key storage |  | `HotWallet` |
...
| 8.3 | Verify correct and atomic nonce management for account-based chains (e.g., EVM): enforce concurrency controls (locks) to avoid nonce reuse or gaps. Ensure the system can issue new transactions with subsequent nonces even while prior ones are unconfirmed (no need to wait for confirmation). Not applicable to UTXO chains (e.g., Bitcoin). | Transaction integrity |  | `HotWallet` |
...
| 8.7 | Maintain an internal transaction ledger that records, at minimum: transaction hash (and replaced-by hash if applicable), timestamp, and lifecycle status (e.g., pending, mined, failed, dropped/evicted, replaced, reorged). This enables accurate reconciliation, alerting, and auditability when transactions are replaced, or chain state changes. | Transaction record completeness |  | `HotWallet` |
...
| 8.11 | Verify safe and idempotent transaction retry mechanisms: for account-based chains, use same-nonce replacements that comply with the chain’s replacement rules; for UTXO chains, use RBF and/or CPFP. Implement dynamic fee bumping. | Transaction integrity |  | `HotWallet` |
